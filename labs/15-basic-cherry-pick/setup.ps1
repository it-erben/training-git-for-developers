...
Set-Content -Path names.txt -Value "Ben`nTom`nSally"
git add names.txt
...
Set-Content -Path animals.txt -Value "Cat`nDog`nMouse"
git add animals.txt
...
Add-Content -Path names.txt -Value "Craig`nJodie`nNathan"
git add names.txt
git commit -m "Commit F: Updated and added more names to the file"

Add-Content -Path sentence.txt -Value "Finally I think this is probably the last sentence to add"
git add sentence.txt
